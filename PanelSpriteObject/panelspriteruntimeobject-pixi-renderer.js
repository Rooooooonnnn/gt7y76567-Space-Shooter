...
(function(gdjs2) {
  ...
  class PanelSpriteRuntimeObjectPixiRenderer {
    constructor(runtimeObject, runtimeScene, textureName, tiled) {
      ...
      runtimeScene.getLayer("").getRenderer().addRendererObject(this._wrapperContainer, runtimeObject.getZOrder());
    }
    getRendererObject() {
    ...
    updateAngle() {
      this._wrapperContainer.rotation = gdjs2.toRad(this._object.angle);
    }
    updatePosition() {
      this._wrapperContainer.position.x = this._object.x + this._object._width / 2;
      this._wrapperContainer.position.y = this._object.y + this._object._height / 2;
    }
    ...
    setColor(rgbColor) {
      ...
      this._centerSprite.tint = gdjs2.rgbToHexNumber(parseInt(colors[0], 10), parseInt(colors[1], 10), parseInt(colors[2], 10));
      for (let borderCounter = 0; borderCounter < this._borderSprites.length; borderCounter++) {
        this._borderSprites[borderCounter].tint = gdjs2.rgbToHexNumber(parseInt(colors[0], 10), parseInt(colors[1], 10), parseInt(colors[2], 10));
      }
      this._spritesContainer.cacheAsBitmap = false;
    }
    getColor() {
